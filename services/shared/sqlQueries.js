...
class SqlQueries {	
	...
	static async getActiveSyncStatus(clusterID){	
		let [condition] = [''];
		if(clusterID) condition = " and cluster.SerialID = '"+clusterID+"'";		
		let query = `SELECT cluster.ID, cluster.Name, cluster.RoleARN, cluster.ExternalID, cluster.TenantID, cluster.SubscriptionID, cluster.ClientID, cluster.ClientSecret, cluster.GrantType, cluster.Resource, cluster.GcpCredentials, cluster.ProviderID as ProviderID, provider.Name as ProviderName, company.ID as CompanyID, user.EmailAddress FROM Clusters cluster Inner Join Providers provider On cluster.ProviderID = provider.ID Inner Join Companies company On company.ID = cluster.CompanyID Inner Join Users user On cluster.EmailTo = user.ID WHERE cluster.IsActive='Yes' and cluster.IsDeleted='No' and cluster.SyncStatus='Yes' and company.IsActive='Yes' and company.IsLocked='No' ${condition}`;
